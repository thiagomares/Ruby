...
class Cadastro
  ...
  def inserir
    ...
    connector = SQLite3::Database.new 'teste.db'
    connector.execute 'INSERT INTO teste (nome, endereço, email, cpf, cep) VALUES (?, ?, ?, ?, ?)', nome, endereco,
                      email, cpf, cep
  end
  ...
  def busca_cadastro_endereco
    user = []
    for i in @cadastro
      user.append(i[:nome]) if i[:CEP] == cep
    end
    if user == []
      puts 'Usuário não encontrado'
    else
...
validador = Cadastro.new
# validador.cria_tabela
validador.coleta_dados
# validador.inserir
validador.cep = '30850650'
validador.busca_cadastro_endereco